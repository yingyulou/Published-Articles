...
alignas(0x1000) uint8_t apStack[(__CPU_COUNT - 1) * 0x1000];
alignas(0x8) uint64_t GDT[7 + __CPU_COUNT * 2];
Queue __taskQueue;
...
void __kernelTaskInit()
{
    ...
    bspTask->__idleTask  = (uint64_t)bspTask;

    memset(bspTSS, 0x0, 104);
    *(uint16_t *)(bspTSS + 102) = 103;
    __makeSegDesc(GDT + 7, GDT + 8, (uint64_t)bspTSS, 103, 0x89);

    for (uint64_t idx = 0; idx < __CPU_COUNT - 1; idx++)
    {
        TCB *apTask    = (TCB *)(apStack + idx * 0x1000);
        uint8_t *apTSS = apTask->__TSS;

        apTask->__CR3       = 0x100000;
        apTask->__taskState = __TASK_READY;
        apTask->__idleTask  = (uint64_t)apTask;

        *(uint16_t *)(apTSS + 102) = 103;
        __makeSegDesc(GDT + 9 + idx * 2, GDT + 9 + idx * 2 + 1, (uint64_t)apTSS, 103, 0x89);
    }
...
TCB *getNextTask()
{
    ...
    for (uint64_t _ = 0, queueSize = queueGetSize(&__taskQueue); _ < queueSize; _++)
    {
